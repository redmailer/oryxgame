...
bool EpollHandler::EpollRegister(INT32 fd, INT32 eventtype /*= EPOLLIN | EPOLLET*/)
{
	...
	ev.data.fd = fd;
	ev.events = eventtype;				//EPOLLET 边缘    EPOLLLT水平
...
bool EpollHandler::doRead(INT32 fd)
{
	...
	do {
		...
		else if (recvNum == 0) {			//出错了
			result = false;
			break;
		}
		else {
			deviceInfo->recv_end += recvNum;
			//前四个字节代表整个包的长度，至少收4个字节再开始解包
...
bool EpollHandler::doWrite(INT32 fd)
{
	...
	do {
		...
		if (sendLen < 0) {
			INT32 iErrorNo = errno;
			if (iErrorNo == EAGAIN) {		//重新发
				break;
			}
			else if (iErrorNo == EWOULDBLOCK) {
				break;
			}
			else if (iErrorNo == EINTR) {	//发完了
				break;
			}
			else {
				result = false;
